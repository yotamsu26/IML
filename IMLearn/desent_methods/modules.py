...
class LogisticModule(BaseModule):
    ...
    Represents the function: f(w) = - (1/m) sum_i^m[y*<x_i,w> -
    log(1+exp(<x_i,w>))]
    """
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray, **kwargs) \
        ...
        output = np.sum(y * prediction - np.log(1 + np.exp(prediction)))
        output = (-1 / y.size) * output
        return output
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) \
        ...
        """
        y = np.array(y)
        exp_weights = np.exp(self.weights@X.T)
        normalized_part = exp_weights / (1 + exp_weights)
        derivative = - (y@X - X.T@normalized_part) / X.shape[0]
        return derivative
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        f_x = fidelity_out + self.lam_ * regularization_out
        return f_x
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        fidelity_der = self.fidelity_module_.compute_jacobian(**kwargs)
        regularization_der = self.regularization_module_.compute_jacobian(
            **kwargs)

        if self.include_intercept_:
            regularization_der = np.insert(regularization_der, 0, 0)

        return fidelity_der + self.lam_ * regularization_der
