...
class GradientDescent:
    ...
    def fit(self, f: BaseModule, X: np.ndarray, y: np.ndarray)->None:
        """
        ...
        """
        best_weights, avg, iteration, best_score = f.weights_, f.weights,\
                                                   0, float('inf')

        for t in range(self.max_iter_):
            prev_weights = f.weights
            eta = self.learning_rate_.lr_step(t=t)
            f.weights = prev_weights - eta * f.compute_jacobian(X=X, y=y)
            ...
            if self.out_type_ == 'average':
                avg += f.weights
                iteration += 1
            if self.out_type_ == 'best' and val < best_score:
                best_score = val
                best_weights = f.weights

            self.callback_(solver=self, weights=f.weights_, val=val, grad=grad,
                           t=t, eta=eta, delta=delta, module=f)
