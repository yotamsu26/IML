...
class LDA(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        self.pi_, self.classes_ = counts / len(y), np.unique(y)

        self.mu_ = np.zeros((len(self.classes_), X.shape[1]))
        for k, c in enumerate(self.classes_):
            self.mu_[k] = np.mean(X[y == c], axis=0)

        new_x = X - self.mu_[y.astype(int)]
        outer_prod = np.einsum("ki,kj->kij", new_x, new_x)
        self.cov_ = outer_prod.sum(axis=0)/(len(X)-len(self.classes_))
    ...
    def likelihood(self, X: np.ndarray) -> np.ndarray:
        ...
        if not self.fitted_:
            raise ValueError("Estimator must first be fitted before calling"
                             " `likelihood` function")
        d = len(X[1])
        z = np.sqrt(((2*np.pi)**d) * np.linalg.det(self.cov_))
        new_x = X[:, None, :] - self.mu_
        exp = np.exp(-0.5 * np.sum(new_x.dot(self._cov_inv)*new_x, axis=2))
        return (self.pi_ * exp) / z
